...
There is nothing to stop you using Discord while running Roblox, and for many purposes (such as talks) it is already very useful even if there is no integration whatsoever with Roblox (so everyone is in a single voice channel). This would be the fallback position if the integration with Discord is found to be against the rules by Roblox administrators. Ultimately metauni is an idea, one particular implementation of which uses Roblox + Discord. Hopefully this implementation remains viable, but the idea can survive the death of any particular implementation.
