...
* In Roblox Studio, insert a whiteboard with the replay functionality. Make sure that under `Settings > Security` you enable Studio Access to API Services, because the recording is stored to a DataStore.
* Click `Play` and open the whiteboard. When you're ready to start recording click "Start Record" in the running Roblox Studio instance, and also start recording audio (I use Camtasia, but your favourite audio recording software will do). When you're finished open the Record menu again and click "Stop Record" and stop recording the audio. Keep in mind that Roblox will only allow you to upload audio segments of seven minutes or less (I try to keep it under 5 minutes to be safe, large files are also rejected).
