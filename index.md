...
The purpose of this page is to help others set up nodes using metauni tools (we call these *outer nodes* as we do not control them) demonstrate best practices through our own nodes (*inner nodes*, which we control) and coordinate the development of open source tools (e.g. for uploading preprints and talk slides) and host announcements of virtual events. Code and demonstration levels are hosted [on Github](https://github.com/metauni/metauni-dev) and contributors are welcome. 
